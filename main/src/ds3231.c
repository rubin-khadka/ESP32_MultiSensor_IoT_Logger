...
esp_err_t DS3231_Init(void) {
	...
	if (I2C_Read_Reg(I2C_BUS_1, DS3231_ADDR, DS3231_REG_STATUS, &status) !=
		ESP_OK) {
	...
	if (is_first_boot) {
		...
		for (int i = 0; i < 7; i++) {
			I2C_Write_Reg(I2C_BUS_1, DS3231_ADDR, DS3231_REG_SECONDS + i,
						  time_data[i]);
		}
	}
...
esp_err_t DS3231_GetTime(DS3231_Time_t *time) {
	...
	// Brust read 7 bytes
	if (I2C_Read_Brust(I2C_BUS_1, DS3231_ADDR, DS3231_REG_SECONDS, data, 7) !=
		ESP_OK) {
	...
	ESP_LOGI(TAG, "%02d/%02d/20%02d %02d:%02d:%02d", time->dayofmonth,
...
esp_err_t DS3231_SetTime(DS3231_Time_t *time) {
	...
	ESP_LOGI(TAG, "Time set to: %02d/%02d/20%02d %02d:%02d:%02d",
			 time->dayofmonth, time->month, time->year, time->hours,
...
float DS3231_GetTemperature(void) {
	...
	if (I2C_Read_Reg(I2C_BUS_1, DS3231_ADDR, DS3231_REG_TEMP_MSB, &temp_msb) !=
		ESP_OK) {
		return 0.0f;
	}
